...
fn main() {
    ...
    // Join both threads and store the value calculated
    let x1_sum = x1_thread.join().expect("x1_thread failed");
    let x2_sum = x2_thread.join().expect("x2_thread failed");

    // Add both calculated values to intermediate_sums
    intermediate_sums.push(x1_sum);
    intermediate_sums.push(x2_sum);
    ...
    // Create a vector for intermediate sums
    let mut intermediate_sums : Vec<usize> = Vec::new();
    
    // Join all threads adding the return value to the intermediate sums vector
    for handler in handlers{
        let partition_sum = handler.join().expect("Thread failed to produce result");
        intermediate_sums.push(partition_sum);
    }
